<%#@~^YwEAAA==@#@&djY,D/`/.P{P^x	HCr	R2Xnm!Y+vEj2d2;K~YrhWW0knY,s]6tPEdnMSGobU,uAIAPEk+Mmk9PxPr[L;k+D&9#@#@&d&o~1}K,DdjdnMR3rw~K_2H@#@&ddPrs+606dYxM/`/+MR6r+^Nd`rYrhW06d+DJ#@#@&72d?A@#@&d7Pb:nr6W/Y@#@&i219~&s@#@&ijY~M/`/+MP{~xKY4k	o@#@&i@#@&i\k	EYd~',IKEUNcPb:nr6W/Y~M,v!b@#@&@#@&@#@&71E.M+	YYb:~',xGhv#@#@&imEM.+	YYbhnP{PGlO+)[9`ExrSHbx;O/~^;MDnxDOb:n*@#@&RGQAAA==^#~@%>
...
function AttendancePunches()
{
  var Punches = new String();
	if (Dept != "") 
	{
	  if (DeptType == 1)
		{
			Punches = "<!--<%=#@~^JgAAAA==];;/DR?D7nDjl.km4snk`J"3H}K2|)9f"J*Ag0AAA==^#~@%>--><!--DeptTransfer="+ClockID+","+Card+","+PunchDate()+","+PunchTime()+","+Dept+"-->";
	 	  }
		else
		{
		  Punches = "<!--<%=#@~^JgAAAA==];;/DR?D7nDjl.km4snk`J"3H}K2|)9f"J*Ag0AAA==^#~@%>--><!--Swipe="+ClockID+","+Card+","+PunchDate()+","+PunchTime()+","+IOStatus+","+LBStatus+"-->";
	...
	if (Tips != "")
	{
		Punches = "<!--<%=#@~^JgAAAA==];;/DR?D7nDjl.km4snk`J"3H}K2|)9f"J*Ag0AAA==^#~@%>--><!--Tips="+ClockID+","+Card+","+PunchDate()+","+PunchTime()+","+Tips+"-->";
	}
...
function JTSPunches()
{
  var Punches = new String();
	...
	Punches = "<!--<%=#@~^JgAAAA==];;/DR?D7nDjl.km4snk`J"3H}K2|)9f"J*Ag0AAA==^#~@%>--><!--Job="+ClockID+","+Card+","+PunchDate()+","+PunchTime()+","+Job+","+JobQty+","+Task+","+TaskQty+","+IOStatus+"-->";
  return Punches;
}
	...
	if (Status > 4) //JTS Entry
	{
	  if (Job == "") 
		...
		if (TaskQty != "")
		{
		  if (Task == "")
			{
		    alert("You Must Enter A <%=#@~^EgAAAA==!YFXSWD9`TVKm/V#uwYAAA==^#~@%> Number To Enter <%=#@~^EgAAAA==!YFXSWD9`TVKm/V#uwYAAA==^#~@%> Quantities");
			  return;
			}
			if(Task=="0")
			{
